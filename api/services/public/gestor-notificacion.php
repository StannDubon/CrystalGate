    ...
    if($_SESSION['idUsuario']){
        switch ($_GET['action']) {
            ...
            case 'createRow':
                ...
                } elseif ($GestorNotificacion->createRow()) {
                    $result['status'] = 1;
                    $result['message'] = 'Manager created succesfully';
                } else {
                    $result['error'] = 'An error ocurred while creating the manager';
                }
                break;
            // Caso para leer todos los registros del usuario.
            case 'readAllByUser':
                if(
                    !$GestorNotificacion->setIdUsuario($_SESSION['idUsuario'])
                ){
                    $result['error'] = $GestorNotificacion->getDataError();
                }
            ...
            case 'changeState':
                ...
                if (
                    ...
                    $result['error'] = $GestorNotificacion->getDataError();
                } elseif ($GestorNotificacion->changeState()) {
                    $result['status'] = 1;
                    $result['message'] = 'Manager edited succesfully';
                    // Se asigna el estado del archivo después de actualizar.
                } else {
                    $result['error'] = 'An error ocurred while editing the Manager';
                }
                break;
