...
const BottomSheet = ({ visible, onClose, onFilterChange }) => {
    ...
    const [selectedDeliverCenter, setSelectedDeliverCenter] = useState('0');
    ...
    const loadData = async () => {
        try {
            ...
            if (requestTypeResult.status) {
                setRequestsType(requestTypeResult.dataset.map(item => ({
                    identifier: item.id_tipo_peticion,
                    value: item.tipo_peticion
                })));
            }

            const languagesResult = await fetchData('idioma', 'readAll');
            if (languagesResult.status) {
                setLanguages(languagesResult.dataset.map(item => ({
                    identifier: item.id_idioma,
                    value: item.idioma
                })));
            }
            ...
            if (centersResult.status) {
                setDeliverCenters(centersResult.dataset.map(item => ({
                    identifier: item.id_centro_entrega,
                    value: item.centro_entrega
                })));
            }
        } catch (error) {
    ...
    useEffect(() => {
        ...
        onFilterChange( selectedRequestType, selectedLanguage, selectedDeliverCenter );
        console.log("DESDE BOTTOMSHEET "+ selectedRequestType, selectedLanguage, selectedDeliverCenter);
                ...
                <View style={styles.body}>
                    <ScrollView contentContainerStyle={styles.scrollContent}>
                        <View style={styles.topContent}>
                            <ComboBox label={"DOCUMENT TYPE"} options={requestsType}
                                placeholder={"All"}
                                selectedValue={selectedRequestType}
                                onValueChange={setSelectedRequestType}
                                isDisabled={false}></ComboBox>
                            <ComboBox label={"LANGUAGE"} options={languages}
                                placeholder={"All"}
                                selectedValue={selectedLanguage}
                                onValueChange={setSelectedLanguage}
                                isDisabled={false}></ComboBox>
                            <ComboBox label={"DELIVERY CENTER"} options={deliverCenters}
                                placeholder={"All"}
